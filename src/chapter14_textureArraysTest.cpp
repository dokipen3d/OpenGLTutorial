...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        //        glm::mat4 view =
        //            glm::lookAt(glm::vec3(std::sin(currentTime * 0.5f) * 4, ((std::sin(currentTime * 0.5f) + 1.0f)
        //            / 2.0f) * 2,
        ...
        view = glm::lookAt(
            glm::vec3(16.f, 2.f, 10.f) +
                glm::vec3(std::sin(currentTime * 0.5f) * 3, ((std::sin(currentTime * 0.5f) + 1.0f) / 2.0f) * 1,
                          std::cos(currentTime * 0.5f) * 3), // Camera is at (4,3,3), in World Space
            glm::vec3(12.27f, 0.9f, 4.83f),                  // and looks at the origin
            glm::vec3(0, 1, 0)                               // Head is up (set to 0,-1,0 to look upside-down)
        );
